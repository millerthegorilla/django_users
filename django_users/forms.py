...
class CustomUserCreation(auth.forms.UserCreationForm):
    captcha = captcha_fields.ReCaptchaField(
        label="", widget=captcha_widgets.ReCaptchaV2Checkbox
    )
    email = forms.EmailField()

    class Meta(auth.forms.UserCreationForm.Meta):
        fields = auth.forms.UserCreationForm.Meta.fields + (
            "email",
            "captcha",
        )
    ...
    def clean_email(self) -> str:
        ...
        try:
            auth.get_user_model().objects.get(email=email)
...
class UserPasswordReset(auth.forms.PasswordResetForm):
    ...
    def clean_email(self) -> str:
        ...
        try:
            ...
            self.add_error(
                "email",
                "Hey!, you haven't registered, perhaps you want to try \
                                     registering first?",
            )
            return email
        return email
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.helper.layout = helper.Layout(
            bootstrap5.FloatingField(
                "username", wrapper_class="col-auto", css_class="mb-3"
            ),
            layout.Field("captcha"),
            layout.HTML(
                '<div class="row justify-content-end"> \
